...
function isNormalizedPackage(value: unknown): value is NormalizedSkillPackage {
  if (!isRecord(value) || value.package_format !== 'projectmind.normalized/v1') return false
...
function isCapabilityBlueprint(value: unknown): value is CapabilityBlueprintView {
  if (!isRecord(value) || value.blueprint_version !== 'projectmind.capability-blueprint/v1') {
    return false
  }
