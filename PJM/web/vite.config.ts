...
function normalizeContextPath(value: string): string {
  ...
  if (!/^\/[A-Za-z0-9._~-]+(?:\/[A-Za-z0-9._~-]+)*$/.test(normalized)) {
    throw new Error(`PROJECTMIND_CONTEXT_PATH must be an absolute path without a trailing slash: ${value}`)
  }
  return normalized
}
...
export default defineConfig(({ mode }) => {
  ...
  const contextPath = normalizeContextPath(env.PROJECTMIND_CONTEXT_PATH ?? '/projectmind')
