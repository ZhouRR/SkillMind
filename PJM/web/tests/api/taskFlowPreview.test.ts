...
describe('strict original task flow preview', () => {
  ...
  it('rejects deeply nested contracts before unbounded recursion', () => {
    ...
    expect(() => parseTaskFlowPreview(changed('plan.task.value.parameter_contract', { ...value, contract_version: 'projectmind.task-contract-draft/v1' }), FLOW_PROJECT, FLOW_TARGET)).toThrow()
  })
  it('rejects aggregate contract field count over 100', () => {
    ...
    const body = { contract_version: 'projectmind.task-contract-draft/v1', type: 'object', fields: ['a', 'b'].map((key) => ({ key, type: 'object', required: true, fields })) }
  ...
  it('accepts the original numeric contract without pretending to recompute Python serialization', () => {
    const payload = changed('plan.task.value.parameter_contract', { contract_version: 'projectmind.task-contract-draft/v1', type: 'number', minimum: 0.0000001, maximum: 1.0 })
...
describe('lossless numeric declarations through the real JSON HTTP boundary', () => {
  ...
  const declaration = (type: string, values: string): string => `{"contract_version":"projectmind.task-contract-draft/v1","type":"${type}","enum":${values}}`
  it.each([
  ...
  it('retains exact bounds and nested result contracts in the same decoded response', async () => {
    const contract = `{"contract_version":"projectmind.task-contract-draft/v1","type":"object","fields":[{"key":"values","type":"array","required":true,"items":{"type":"number","minimum":-9007199254740993,"maximum":${huge},"enum":[9007199254740992,9007199254740993,1.0,1e-7,-0.0]}}]}`
  ...
  it.each([
    ...
    const raw = `{"contract_version":"projectmind.task-contract-draft/v1","type":"object","fields":[${inner}]}`
  ...
  it.each([[0, 0, true], [2, 1, false], [100000, 100000, true]])('checks original string lengths %s <= %s', async (minimum, maximum, valid) => {
    const raw = `{"contract_version":"projectmind.task-contract-draft/v1","type":"string","min_length":${minimum},"max_length":${maximum}}`
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(flowWireWithContract(raw), { status: 200 })))
