...
from projectmind.core.secret_crypto import EncryptedSecret
...
REPOSITORY_WRITE_BRANCH_PREFIX = "projectmind/"
_BRANCH_PREFIX_PATTERN = re.compile(r"^projectmind/[A-Za-z0-9][A-Za-z0-9._/-]{0,110}$")
...
PROVIDER_DEFINITIONS: dict[str, ProviderDefinition] = {
    ...
    "svn": ProviderDefinition(
        ...
        # 平台固定の約定 (`branches/projectmind/`) であり、仓库布局を推測しない。
        capabilities=frozenset({"repository.read/v1", "repository.write/v1"}),
        write_capabilities=frozenset({"repository.write/v1"}),
        requires_secret=True,
    ...
    if provider in {"git", "svn"}:
        ...
        if prefix is not None:
            ...
            if (
                ...
                raise IntegrationValidationError(
                    "Repository write branch prefix must narrow the projectmind/ namespace"
                )
