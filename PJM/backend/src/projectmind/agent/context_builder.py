...
from projectmind.agent.repository_provider import RepositoryReadProvider
...
from projectmind.agent.task_brief import (
...
class ContractStore:
    ...
    @property
    def root(self) -> Path:
        """Fixture Provider と共有する read-only contracts root を返す。"""
...
class _BoundRepositoryProvider:
    """Integration 束縛の有無で fixture と実 client を振り分ける repository Provider。

    歴史的 M0 受入経路 (LEGACY_FIXTURE 選択で integration_id を持たない Run) は固定 fixture を
    読み、Integration 束縛のある Run だけが実 repository へ出る。両者は provider 名 `git` を
    共有するため、Run snapshot の事実 (束縛の有無) で決定的に分岐させる。
    """

    def __init__(self, *, fixture: ToolProvider, bound: ToolProvider) -> None:
        """Fixture Provider と実 repository Provider を保持する。"""
    ...
    async def execute(
        ...
        """束縛済み Run は実 repository、未束縛 Run は fixture へ委譲する。"""

        if context.tool.integration_id is None or context.tool.binding_id is None:
            return await self._fixture.execute(context, arguments)
        return await self._bound.execute(context, arguments)


def _fixture_tool_definitions(
    contracts: ContractStore,
    *,
    additional_issue_providers: Mapping[str, ToolProvider] | None = None,
    repository_source: RepositorySnapshotSource | None = None,
) -> tuple[ToolDefinition, ...]:
    """業務 Skill に依存しない CSV/Git fixture Tool 定義を組み立てる。

    ``repository_source`` を渡すと repository.read/v1 に実 git/svn Provider を配線する
    (計画 §19 W4)。渡さない offline 構成では fixture だけが残る。
    """

    fixture_root = contracts.root / "fixtures" / "generic-read-providers"
    issue_provider = CsvFixtureIssueProvider(fixture_root / "issues.csv")
    repository_fixture = GitFixtureRepositoryProvider(fixture_root / "repository")
    repository_providers: dict[str, ToolProvider] = {"git": repository_fixture}
    if repository_source is not None:
        bound = RepositoryReadProvider(repository_source)
        repository_providers = {
            "git": _BoundRepositoryProvider(fixture=repository_fixture, bound=bound),
            "svn": bound,
        }
    return (
        ToolDefinition(
            capability="issue.read/v1",
            description="Read one issue from the bound CSV fixture",
            ...
            providers={"csv": issue_provider, **dict(additional_issue_providers or {})},
        ),
        ToolDefinition(
...
def create_fixture_tool_registry(contracts: ContractStore) -> ToolRegistry:
    """Offline/初期配備で許可する汎用 CSV/Git fixture 能力を登録する。"""

    return ToolRegistry(
        (
            *_fixture_tool_definitions(contracts),
...
def create_run_tool_registry(
    ...
    *,
    ...
    """Fixture 能力に加え、project 文書を読む document.read/v1 を registry へ登録する。"""

    return ToolRegistry(
        (
            *_fixture_tool_definitions(
                contracts,
                additional_issue_providers=(
                    {"redmine": redmine_issue_provider}
...
class DeferredInteractionProvider:
    ...
    async def execute(
        ...
        raise ToolProviderError(
            "unavailable",
            "Interaction request must be deferred by ProjectMind",
            retryable=False,
        )
...
class DeferredChangeProposalProvider:
    ...
    async def execute(
        ...
        raise ToolProviderError(
            "unavailable",
            "ChangeProposal request must be deferred by ProjectMind",
            retryable=False,
        )
...
def _change_propose_tool_definition(contracts: ContractStore) -> ToolDefinition:
    ...
    return ToolDefinition(
        ...
        description=(
            ...
            "ProjectMind independently validates approval and scope"
        ),
...
class ProductionRunContextBuilder:
    ...
    async def build(self, claimed_run: ClaimedRun, *, sequence_start: int) -> RunContext:
        ...
        # JAF 固定 task に依存せず、任意 published task の Run を同一経路で実行できるようにする。
...
def _resolve_source_tools(
    ...
    *,
    ...
    JAF 固定 capability には依存せず、blueprint の宣言順に要求を走査する。必須要求が未選択
...
def _selected_source(
    ...
    if raw_integration_id is None:
        accepted = frozenset(
            item
            for item in _sequence(requirement.get("accepted_providers"))
            if isinstance(item, str)
        )
        if provider not in accepted:
            raise ValueError("Selected legacy data source provider is not accepted")
