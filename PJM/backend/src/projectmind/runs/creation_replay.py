...
from projectmind.documents.snapshot import DOCUMENT_READ_CAPABILITY
...
def _legacy_intent(run: Run, command: CreateRunCommand) -> TaskRunIntent:
    ...
    for key, value in command.selected_sources_json.items():
        ...
        if value.get("binding_level") == "LEGACY_FIXTURE":
            sources[key] = _string(value, "provider")
        elif value.get("capability") == DOCUMENT_READ_CAPABILITY:
