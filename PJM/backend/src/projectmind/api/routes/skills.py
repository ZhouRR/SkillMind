...
from projectmind.skills.domain import SkillPreview
from projectmind.skills.realtime import (
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    responses={
        400: {"description": "Skill source cannot be parsed deterministically"},
        503: {"description": "Object storage is not configured for uploads"},
    },
    tags=["skills"],
)
async def upload_skill_import(
    ...
    files: Annotated[
        list[UploadFile],
        File(description="Skill source files; filename carries the relative path"),
    ],
) -> StoredSkillPreviewResponse:
    """ADMIN が multipart で Skill (binary asset 可) を Organization へ保存する。"""

    service: SkillService = request.app.state.skill_service
    upload_files = await _upload_skill_files(files)
    try:
...
async def _upload_skill_files(files: list[UploadFile]) -> tuple[UploadSkillFile, ...]:
    """Multipart UploadFile を読み出し、filename を相対 path とする domain value へ変換する。"""
...
def _storage_unavailable(error: Exception) -> ProblemException:
    """Object storage 未配線を安定した 503 Problem へ変換する。"""
