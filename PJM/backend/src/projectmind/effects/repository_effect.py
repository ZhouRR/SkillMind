...
from projectmind.effects.forge import ForgeTransport, PullRequestRef
...
from projectmind.effects.repository_write import REPOSITORY_WRITE_CAPABILITY
...
class GitRepositoryWriteProvider:
    ...
    async def _open_pull_request(
        ...
        *,
        ...
        title = display if isinstance(display, str) and display else "ProjectMind change"
        return await self._forge_transport.ensure_pull_request(
            ...
            title=title,
            body=(
                f"Applied by ProjectMind after approval.\n\n"
                f"ProjectMind-Proposal: {execution.proposal_ref}\n"
            ),
        )
...
def _commit_message(execution: ClaimedEffectExecution) -> str:
    ...
    return f"{headline}\n\nProjectMind-Proposal: {execution.proposal_ref}\n"
...
class SvnRepositoryWriteProvider:
    ...
    (`<repository root>/branches/projectmind/<名前>`) であり、repository root は `svn info` の
