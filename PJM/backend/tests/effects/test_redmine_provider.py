...
from projectmind.effects.domain import ClaimedEffectExecution
...
async def test_http_transport_uses_versioned_adapter_contract(
    ...
    def request_json(**kwargs: Any) -> dict[str, Any]:
        ...
        if str(kwargs["url"]).endswith("projectmind-effect-provider.json"):
            return {
                "protocol": "projectmind.redmine-effect/v1",
                "provider": "redmine",
    ...
    assert requests[1]["url"].endswith(
        "/projectmind/effects/issue.update/v1/issues/42.json"
    )
