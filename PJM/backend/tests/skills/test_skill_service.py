...
from projectmind.db.models import SkillInterpretation, SkillSource
...
async def test_save_inline_parses_and_persists_deterministic_preview() -> None:
    ...
    session = _Session()
    service = SkillService(_SessionFactory(session), ROOT / "contracts")  # type: ignore[arg-type]
...
class _InterpretFactory:
    ...
    def __init__(self, session: _InterpretSession) -> None:
        """追加 model を検査する session を保持する。"""

        self._session = session

    def __call__(self) -> _InterpretSession:
        """Async context 対応 session を返す。"""
...
def _service(session: _InterpretSession, interpreter: object) -> SkillService:
...
async def test_interpret_reuses_same_frozen_identity_without_model_call() -> None:
    ...
    first = await _service(
        first_session, FixtureSkillInterpreter(_example_response())
    ).interpret(organization_id=organization_id, skill_source_id=source_id, model="claude-opus-4-8")
...
async def test_interpret_force_regenerate_creates_a_new_execution_key() -> None:
    ...
    for _ in range(2):
        ...
        stored = await _service(
            session, FixtureSkillInterpreter(_example_response())
        ).interpret(
            organization_id=organization_id,
