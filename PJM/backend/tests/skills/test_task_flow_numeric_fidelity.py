...
from projectmind.api.routes.task_flow import FlowContractResponse
from projectmind.core.hashing import canonical_json, sha256_hex
from projectmind.skills.task_contract import TaskContractCompilationError, compile_task_contract
...
def _positioned_contract(node: dict[str, Any], position: str) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"contract_version": "projectmind.task-contract-draft/v1"}
    if position == "root":
        result.update(node)
    elif position == "field":
