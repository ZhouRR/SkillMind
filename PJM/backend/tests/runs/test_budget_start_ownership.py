...
from projectmind.db.models import RunBudgetReservation
...
async def test_recorder_uses_distinct_one_shot_tokens_without_leaking_them_to_sdk_or_receipts(
    ...
    monkeypatch.setattr("projectmind.runs.budget_execution.secrets.token_urlsafe", token_urlsafe)
    case = LedgerExecution(tmp_path)
    ...
    monkeypatch.setattr("projectmind.runs.budget_execution.check_pending_cancellation", check)
    await case.reserve()
    await case.execute()
    row = case.database.reservations[0]
...
async def test_changed_process_identity_burns_inherited_recorder_without_sql_or_client(
    ...
    with monkeypatch.context() as changes:
        changes.setattr("projectmind.runs.budget_execution.os.getpid", lambda: original_pid + 1)
        with pytest.raises(BudgetUnavailableError):
            await case.execute()
    assert case.recorder._start_owner_token is None
