...
from projectmind.agent.claude import ClaudeRuntimeConfiguration, build_claude_agent_options
...
def _run_context(tmp_path: Path) -> RunContext:
    ...
    return RunContext(
        ...
        project_id=uuid4(),
        user_id=uuid4(),
        prompt="JAF Ticket を分析する",
        task_snapshot={"capability": "jaf.ticket.analyze"},
        skill_snapshots=({"version": "m0-jaf-quality-analysis/v1"},),
        ...
        tools=(
            RegisteredTool(
                ...
                sdk_name="mcp__projectmind__issue_read_v1",
                provider="redmine",
...
def test_options_disable_builtin_tools_and_local_configuration(tmp_path: Path) -> None:
    ...
    options = build_claude_agent_options(
        ...
        mcp_server={"type": "sdk", "name": "projectmind", "instance": object()},
    ...
    assert options.allowed_tools == ["mcp__projectmind__issue_read_v1"]
...
async def test_pre_tool_hook_registers_authorized_invocation(tmp_path: Path) -> None:
    ...
    input_data: dict[str, Any] = {
        ...
        "transcript_path": "/tmp/transcript",
        "cwd": str(options.cwd),
        "tool_name": "mcp__projectmind__issue_read_v1",
        "tool_input": {"issue_ref": "JAF-1", "purpose": "analysis"},
        "tool_use_id": "tool-1",
    }
    ...
    assert captured == [
        (
            "mcp__projectmind__issue_read_v1",
            {"issue_ref": "JAF-1", "purpose": "analysis"},
            "tool-1",
            session_id,
        )
    ]
...
def test_runtime_environment_does_not_forward_unlisted_values(
    ...
    monkeypatch.setenv("PROJECTMIND_DATABASE_URL", "must-not-be-forwarded")

    configuration = ClaudeRuntimeConfiguration.from_environ()

    assert configuration.environment["ANTHROPIC_AUTH_TOKEN"] == "test-token"
    assert "PROJECTMIND_DATABASE_URL" not in configuration.environment


def test_anthropic_model_is_the_primary_model_source(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Custom endpoint の model ID を ProjectMind 独自設定で上書きしない。"""
...
def test_options_blank_worker_credentials_in_sdk_subprocess_environment(
    ...
    monkeypatch.setenv("PROJECTMIND_DATABASE_URL", "must-not-reach-agent")
    options = build_claude_agent_options(
        _run_context(tmp_path),
        mcp_server={"type": "sdk", "name": "projectmind", "instance": object()},
        configuration=ClaudeRuntimeConfiguration(environment={}),
    ...
    assert options.env["PROJECTMIND_DATABASE_URL"] == ""
...
def test_runtime_configuration_rejects_explicit_unlisted_key() -> None:
    ...
    with pytest.raises(ValueError, match="Unsupported Claude runtime environment"):
        ClaudeRuntimeConfiguration(environment={"PROJECTMIND_DATABASE_URL": "secret"})
