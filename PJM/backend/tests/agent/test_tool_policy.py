...
from projectmind.agent.tool_policy import (
    ToolExecutionPolicy,
    ToolPolicyViolation,
...
def test_registered_tool_with_valid_input_is_allowed() -> None:
    ...
    tool = policy.authorize(
        ...
        {"issue_ref": "JAF-1234", "purpose": "品質分析"},
    )

    assert tool.capability == "issue.read/v1"
    assert policy.allowed_sdk_names == ("mcp__projectmind__issue_read_v1",)
...
def test_schema_violation_is_denied() -> None:
    ...
    with pytest.raises(ToolPolicyViolation, match="registered schema"):
        policy.authorize(
            "mcp__projectmind__issue_read_v1",
            {"issue_ref": "JAF-1234", "purpose": "品質分析", "unexpected": True},
        )
