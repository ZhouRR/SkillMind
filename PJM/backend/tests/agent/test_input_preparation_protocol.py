...
from projectmind.agent.repository_source import RepositoryBindingRef
from projectmind.agent.workspace import WorkspaceManager
from projectmind.agent.workspace_materializer import MaterializationError, PreparedInput
from projectmind.core.hashing import sha256_hex
from projectmind.documents.snapshot import DocumentSnapshot
from projectmind.runs.domain import ClaimedRun, LeaseValidationError
from projectmind.runs.input_snapshot import InputFileSeal, InputSnapshotStatus
...
def _physical_files(workspace: RunWorkspace) -> set[str]:
    ...
    namespace = workspace.root / ".projectmind-inputs"
    return {
...
async def test_preparation_never_resigns_an_orphan_namespace(tmp_path: Path, orphan: str) -> None:
    ...
    namespace = case.workspace.root / ".projectmind-inputs"
    if orphan == "file":
...
async def test_missing_required_repository_fails_before_receipt_or_content(tmp_path: Path) -> None:
    ...
    assert not (case.workspace.root / ".projectmind-inputs").exists()
...
async def test_file_and_manifest_co_tampering_cannot_replace_trusted_receipt(
    ...
    prepared = await case.prepare()
    receipt = case.store.record
    manifest_path = prepared.workspace.input_dir / "repo_a/.projectmind/manifest.json"
