...
EXAMPLES = {
    ...
    "examples/generated-task-manifest.v1alpha1.json": (
        "runtime-manifest/v1alpha1.schema.json"
    ),
    "examples/generic-native-manifest.v1alpha1.json": (
        "runtime-manifest/v1alpha1.schema.json"
    ),
...
def test_runtime_outcome_schema_matches_public_contract() -> None:
    ...
    assert load(CONTRACTS / "outcomes" / "envelope" / "v1.schema.json") == (
...
def test_runtime_interaction_schema_matches_public_contract() -> None:
    ...
    from projectmind.runs.interaction import INTERACTION_REQUEST_SCHEMA
...
def test_runtime_change_proposal_schema_matches_public_contract() -> None:
    ...
    from projectmind.effects.proposal import CHANGE_PROPOSE_REQUEST_SCHEMA
...
def test_every_example_on_disk_is_registered() -> None:
    ...
    on_disk = {
        f"examples/{path.name}" for path in sorted((CONTRACTS / "examples").glob("*.json"))
    }
...
def test_exported_openapi_is_current() -> None:
    ...
    exported = load(CONTRACTS / "openapi" / "projectmind-api.v1.json")
