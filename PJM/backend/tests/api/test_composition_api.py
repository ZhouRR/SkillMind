...
def test_update_module_preserves_public_fields_and_exact_bindings(client: TestClient) -> None:
    """ADMIN の更新は既存 body/response のまま精確版集合を置き換える。"""

    assert isinstance(client.app, FastAPI)
    fake = FakeCompositionService()
    client.app.state.composition_service = fake
    project_id, module_id, version_id = uuid4(), uuid4(), uuid4()
    response = client.put(
        f"/api/v1/projects/{project_id}/modules/{module_id}",
        json={
            "name": "Synthetic updated module",
            "description": "Synthetic updated description",
            "skill_version_ids": [str(version_id)],
        },
    )
    ...
    assert response.json()["description"] == "Synthetic updated description"
    assert fake.updated == [(module_id, "Synthetic updated module", [version_id])]
    assert len(fake.accesses) == 1
