...
def validate() -> None:
    ...
    if "traefik" in services:
        raise ValueError("ProjectMind must use the existing Traefik, not define one")
    ...
    context_expression = "${PROJECTMIND_CONTEXT_PATH:-/projectmind}"
    api_rule = str(api_labels.get("traefik.http.routers.projectmind-api.rule"))
    web_rule = str(web_labels.get("traefik.http.routers.projectmind-web.rule"))
    ...
    api_strip_prefix = api_labels.get(
        "traefik.http.middlewares.projectmind-api-context.stripprefix.prefixes"
    )
    web_strip_prefix = web_labels.get(
        "traefik.http.middlewares.projectmind-web-context.stripprefix.prefixes"
    )
    ...
    if backend_environment.get("PROJECTMIND_CONTEXT_PATH") != context_expression:
    ...
    if web_build_args.get("PROJECTMIND_CONTEXT_PATH") != context_expression:
        raise ValueError("Web build must receive the same context path used by Traefik")
    tls_expression = "${TRAEFIK_TLS:-true}"
    if api_labels.get("traefik.http.routers.projectmind-api.tls") != tls_expression:
        raise ValueError("API router TLS mode must remain environment-configurable")
    if web_labels.get("traefik.http.routers.projectmind-web.tls") != tls_expression:
        raise ValueError("Web router TLS mode must remain environment-configurable")
    if api_labels.get("traefik.http.routers.projectmind-api.priority") != "100":
