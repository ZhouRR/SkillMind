...
description: 実行前に固定した RV 済み Markdown と Runner の原操作記録・画像を直接照合し、利用可能な業務知識で条件を解釈して、ケース別 Verdict と原因分類を MinIO・PostgreSQL に保存する。実行後の判定や失敗のトリアージに使用する。
---
...
判定では知識庫を用語・業務状態・権限・終了条件の理解と原因分類に使う。実行前に確定した期待条件と RV の解釈を維持し、後から取得した規則で合格条件を変更しない。実測との比較は Runner 原記録で行い、知識庫の説明を Evidence や実施済みの証拠にしない。仕様との矛盾や期待条件不足が判定を左右する場合は、その影響を TEST／INCONCLUSIVE 等の既存基準で扱い、製品不具合と即断しない。未収録・参照不能だけで判定可能なケースを止めない。
