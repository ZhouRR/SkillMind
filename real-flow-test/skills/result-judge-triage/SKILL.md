...
description: RV 済み Markdown から作成した固定実行計画・実測結果・Evidence を照合し、ケース別 Verdict と原因分類を MinIO・PostgreSQL に保存する。実行後の判定や失敗のトリアージに使用する。
---
...
必要な入力は `testRunId`、`executionId`、または実行結果のディレクトリ名・ファイル名。実行完了からの継続では追加入力を求めず、計画、期待結果、版、ログ、Evidence を接続済みシステムから取得する。

PostgreSQL、MinIO、Evidence 読み取り、JSON Schema 検証ツールを使う。入力は [execution-result.schema.json](../../schemas/execution-result.schema.json)、計画は [execution-plan.schema.json](../../schemas/execution-plan.schema.json)、出力は [verdict.schema.json](../../schemas/verdict.schema.json)（すべて `schemaVersion: 1.0`）に従う。既知 Issue の参照機能は利用できる場合に使用する。他の Skill は参照・呼び出しせず、工程の起動はワークフロープラットフォームに任せる。
...
3. 外部参照を使わず結果と計画の Schema・format を検証する。結果の `sourcePlan` を実行前に固定した参照および DB の `test_automation.test_execution_plan.plan_ref` と照合し、取得した計画のバイト列の SHA-256 を確認する。実行・文書 ID、`specVersion`、`versionSnapshot` も DB と計画で一致させる。不一致は `BLOCKED` とし、不正な入力から製品不具合を断定しない。
4. 固定計画の全ケース・ステップと期待結果を判定の基準にする。原文の確認が必要なら計画の `sourceMarkdown` と `sourceRefs` を使い、Markdown のハッシュも確認する。候補 IR や Git の正式 IR は要求しない。Runner が返した期待値で基準を置き換えず、欠落ケース・ステップも対象に含め、計画にない ID は不整合とする。
...
新規成果物のディレクトリ名・ファイル名は以下の日本語名を既定とし、明示されたプロジェクト設定を優先する。既存の結果・計画・判定は DB に記録された参照から取得し、再開時も同じ保存先を使う。bucket・ID・JSON フィールド名・Schema 名は変更しない。

1. 出力生成前にケースの `attempt_count` を増やす。Schema に従って `判定結果.json` を生成し、識別子・期待条件・Evidence の対応も検証する。判定と不具合 Payload の `sourcePlan`・`versionSnapshot` は検証済み実行結果から引き継ぎ、判定モデルは別の `judgementModelVersion` に記録する。製品不具合の Payload は同じ Schema の `$defs/defectPayload` で検証する。
...
5. ケース別の Verdict・処理状態・成果物パス・推奨対応を返す。計画の全ケースを母数に集計し、未判定件数も示す。
