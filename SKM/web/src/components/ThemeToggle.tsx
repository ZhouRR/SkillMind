 ...
 *
 *  compact は sidebar 用。縦幅を節約するため文字を視覚的に隠すが、読み上げ名・tooltip・
 *  textContent には外観名を残し、icon だけに意味を持たせない。 */
export function ThemeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={compact ? 'themeToggle themeToggleCompact' : 'themeToggle'} role="group" aria-label={labels.label}>
    {(['light', 'dark'] as const).map((value) => <button type="button" key={value}
      aria-pressed={theme === value} title={compact ? labels[value] : undefined}
      onClick={() => { setTheme(value); saveTheme(value) }}>
      ...
      {compact ? <span className="visuallyHidden">{labels[value]}</span> : labels[value]}
    </button>)}
  </div>
}
