...
export function emptyConnectDraft(provider: ResourceProvider): ConnectDraft {
  return {
    ...
    mcpCatalog: null, mcpTools: false, mcpPermissions: {}, serverUrl: '', tables: '', resourceUris: '', writeColumns: '', databaseOperations: ['INSERT', 'UPDATE'],
...
export function connectDraftFromIntegration(
  ...
  return {
    ...
    mcpPermissions: typeof config.tool_permissions === 'object' && config.tool_permissions !== null
      ? Object.fromEntries(Object.entries(config.tool_permissions).filter((entry): entry is [string, string] => entry[1] === 'read' || entry[1] === 'call')) : {},
