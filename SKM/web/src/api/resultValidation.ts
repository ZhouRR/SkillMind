...
} & ({ version: 'skillmind.result-reference-checks/v1'; artifacts: 'NOT_VERIFIED' }
  | { version: 'skillmind.result-reference-checks/v2'; artifacts: 'RUN_OWNERSHIP_AND_CONTENT' })
...
export function isResultReferenceChecks(
  ...
  return isRecord(value)
    ...
    && (value.version === 'skillmind.result-reference-checks/v1' && value.artifacts === 'NOT_VERIFIED'
      || value.version === 'skillmind.result-reference-checks/v2' && value.artifacts === 'RUN_OWNERSHIP_AND_CONTENT')
...
export function isRunResultValidation(
  ...
  return isResultReferenceChecks(value.reference_checks, resultKind)
    ...
    && (value.reference_checks.version !== 'skillmind.result-reference-checks/v2' || value.artifact_refs_valid === true)
    && (value.reference_checks.version !== 'skillmind.result-reference-checks/v1' || value.artifact_refs_valid !== true)
