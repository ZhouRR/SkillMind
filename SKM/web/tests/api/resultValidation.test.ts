...
const CHECKS: ResultReferenceChecks = {
  version: 'skillmind.result-reference-checks/v1', evidence: 'RUN_OWNERSHIP',
...
describe('saved Result reference-check protocol', () => {
  it('consumes the shared v2 example while preserving its original empty artifact set and data', async () => {
    ...
    expect(detail.result?.validation.reference_checks?.version).toBe('skillmind.result-reference-checks/v2')
  ...
  it.each(['OUTCOME_ENVELOPE', 'STRUCTURED_OUTPUT'] as const)('accepts exact v2 and requires saved artifact validity for %s', async (kind) => {
    const checks = { ...CHECKS, version: 'skillmind.result-reference-checks/v2', artifacts: 'RUN_OWNERSHIP_AND_CONTENT',
  ...
  it.each([
    null, [], true, 'verified', {},
    { ...CHECKS, version: 'skillmind.result-reference-checks/v2' },
