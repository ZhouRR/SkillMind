...
describe('auth API client', () => {
  it.each([true, false, undefined])('accepts or omits the independent scheduling flag %s', async (enabled) => {
    vi.stubGlobal('fetch', vi.fn<typeof fetch>().mockResolvedValue(jsonResponse({ ...SESSION, scheduling_enabled: enabled })))
    expect((await loadAuthSession())?.scheduling_enabled).toBe(enabled)
  })

  it('rejects a non-boolean scheduling flag', async () => {
    vi.stubGlobal('fetch', vi.fn<typeof fetch>().mockResolvedValue(jsonResponse({ ...SESSION, scheduling_enabled: 'true' })))
