...
class InteractionApi:
    ...
    async def route(self, route: Route) -> None:
        ...
        if not address.path.startswith(self.prefix):
            if request.method == "GET" and not address.path.startswith("/skillmind/api/"):
...
def main() -> None:
    ...
    parser.add_argument(
        "--url", default="http://127.0.0.1:5203/skillmind/tests/browser/run-submission.html"
    )
