...
class LoginApi:
    ...
    async def route(self, route: Route) -> None:
        ...
        if request.method == "GET" and url.path == f"{self.api_prefix}v1/auth/login-context":
            ...
            assert f"skillmind_login_csrf={self.tokens[-1]}" in request.headers.get("cookie", "")
        else:
            self.unexpected.append(request.url)
            await route.abort()
            return
        ...
        if status == 200 and stage == "context":
            ...
            headers["Set-Cookie"] = (
                f"skillmind_login_csrf={token}; Max-Age=300; Path=/; HttpOnly; SameSite=Strict"
            )
        elif status == 200:
            body = {
                "user": {
            ...
            headers["Set-Cookie"] = (
                "skillmind_session=test-only; Path=/; HttpOnly; SameSite=Strict"
            )
        else:
            code = {
            ...
            for name, language, width, status, retry, non_json, feedback in cases:
                ...
                try:
                    ...
                    if name.endswith("-abort"):
                        ...
                        assert any(
                            cookie["name"] == "skillmind_session" and cookie["httpOnly"]
                            for cookie in cookies
                        )
                        assert "skillmind_session" not in await page.evaluate("document.cookie")
                    else:
...
def main() -> None:
    ...
    parser.add_argument(
        "--url", default="http://127.0.0.1:5191/skillmind/tests/browser/login.html"
    )
