...
本規約は Skillmind 全体に適用する。コード・起動は [README](README.md)、利用・保守は [docs](../docs/README.md)、詳細な形状と処理は契約・実装・テストを参照し、ここに複製しない。

## 作業前に読むもの

1. [現在の計画](../docs/planning/roadmap.md#当前执行状态)で実装範囲と不足を確認する。規約や test の存在を実装済みの証拠にしない。
2. [変更ガイド](../docs/development/change-guide.md)から対象コードと保守境界を読む。ガイドと実装の矛盾は影響を確認して整合させる。
...
- PostgreSQL は正本、Redis は Queue・短期 Lock・通知。Run snapshot/権限上限/Result は不変。人工修正は Evaluation、人工答復/批准は新 Segment、同 Segment 復旧は新 Attempt。原 Run の自動同意による短操作は、直接交付の所有権を保存した場合のみ同 Attempt に原回执を返せる。
...
- 外部 write は observe → propose → apply。[受控写入](../docs/development/runtime-guide.md#外部效果)の承認、Provider、scope、競合・idempotency・read-back を省略しない。repository.write/v1 は Project 事前許可不可・force 禁止。Git/MCP はそれぞれ原 Run 開始時の明示同意による自動承認を許可する。
- 生成 UI・並行子 Agent・Shell/network の拡張は[後置範囲](../docs/development/runtime-guide.md#后置能力)を確認し、影響する権限・隔離・予算を実装と回帰で検証する。局部実装を全面許可にしない。生成 bundle は CSP sandbox allow-scripts、iframe に allow-same-origin を付けない。子能力は resolve_subagent_capabilities と Run 共通予算を通す。
...
- 完了時はガイド/契約との整合と今回の検証を報告し、mock と実環境の証拠を区別する。計画は機能状態が変わった時だけ更新する。
