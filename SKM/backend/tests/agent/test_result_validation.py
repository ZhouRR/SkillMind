...
async def test_generic_result_falls_back_to_neutral_defaults() -> None:
    """通用 convention field が無い結果は中立な既定値へ畳み、業務固有 文案を出さない。"""
...
@pytest.mark.asyncio
async def test_generic_interpreter_does_not_apply_business_finding_rules() -> None:
    """任意 Schema の field assessment に、未宣言の業務規則を課さない。"""

    # Evidence を必須としない Schema では、空の参照だけを理由に拒否しない。
    validated = await ResultValidator(MemoryEvidenceLookup(frozenset())).validate(
        run_id=uuid4(),
        schema={"type": "object", "additionalProperties": True},
        schema_ref=SCHEMA_CHECKSUM,
