...
async def test_git_credential_is_injected_through_the_environment_not_argv(
    ...
    for item in invocations:
        ...
        assert not any(key.startswith("SKILLMIND_") for key in item["env"])
...
def test_stub_environment_does_not_leak_worker_configuration(
    ...
    monkeypatch.setenv("SKILLMIND_DATABASE_URL", "postgresql://user:password@db/skillmind")
    monkeypatch.setenv("SKILLMIND_MANAGED_SECRET_KEK", "v1:" + "A" * 44)

    from skillmind.agent.repository_client import _git_environment, _svn_environment
    ...
    for environment in (git_environment, svn_environment):
        assert not any(key.startswith("SKILLMIND_") for key in environment)
        assert all("password@db" not in value for value in environment.values())
    assert os.environ["SKILLMIND_DATABASE_URL"].startswith("postgresql://")
