...
from skillmind.agent.evidence import EvidenceRecord, ToolInvocation, invocation_fingerprint
...
from skillmind.agent.workspace_provider import WorkspaceWriteProvider
from skillmind.artifacts.domain import ArtifactDraft, ArtifactMetadata
...
async def test_invalid_audit_locator_is_rejected_before_file_write(
    ...
    write = MagicMock()
    monkeypatch.setattr("skillmind.agent.workspace_provider._write_workspace_file", write)
...
async def test_real_writer_commits_private_bytes_with_same_tool_result(
    ...
    monkeypatch.setattr("skillmind.artifacts.repository.ArtifactRepository.list_metadata", quota)
