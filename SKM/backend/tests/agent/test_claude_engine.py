...
from skillmind.agent.engine import ClaudeAgentSdkEngine, ClaudeMessageMapper
...
def test_runtime_configuration_uses_allowlisted_fallback_without_overriding_environment(
    ...
    configuration = ClaudeRuntimeConfiguration.from_environ(
        fallback={
            ...
            "SKILLMIND_DATABASE_URL": "must-not-be-copied",
        }
    )
    ...
    events = mapper.map(
        AssistantMessage(
            content=[
                ...
                ToolUseBlock(
                    id="tool-1",
                    name="mcp__skillmind__issue_read_v1",
