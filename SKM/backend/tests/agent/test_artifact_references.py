...
async def test_verified_artifacts_are_saved_with_exact_v2_scope(top: bool, nested: bool) -> None:
    ...
    assert (
        result.validation["reference_checks"]["version"] == "skillmind.result-reference-checks/v2"
    )
...
async def test_checkpoint_uses_same_content_verifier(
    ...
    monkeypatch.setattr("skillmind.artifacts.repository.ArtifactRepository.verified_refs", lookup)
    run_id = uuid4()
    checkpoint = {
        "evidence_refs": [],
