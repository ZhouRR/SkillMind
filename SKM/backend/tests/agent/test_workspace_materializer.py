...
from skillmind.agent.workspace_provider import WorkspaceReadProvider, WorkspaceSearchProvider
...
async def test_materializes_explicit_frozen_documents_under_input_documents(tmp_path: Path) -> None:
    ...
    contents = [
        ...
        _doc("", "ISSUE-list.csv", b"key,owner\n1,alice\n"),
    ]
    ...
    assert (base / "ISSUE-list.csv").read_text(encoding="utf-8") == "key,owner\n1,alice\n"
    manifest = json.loads((base / ".skillmind" / "manifest.json").read_text(encoding="utf-8"))
...
async def test_root_budget_overflow_never_publishes_partial_input(tmp_path: Path) -> None:
    ...
    assert (workspace.root / ".skillmind-inputs" / str(store.record.snapshot_id)).is_dir()
...
def _read_context(
    ...
    return RunToolContext(
        ...
        tool=RegisteredTool(
            ...
            sdk_name=f"mcp__skillmind__{capability.replace('.', '_').replace('/', '_')}",
            provider="workspace",
            integration_id=None,
...
@pytest.mark.parametrize(
    "key", ["../escape", "documents", ".", ".skillmind", "bad/key", "bad\\key"]
)
...
async def test_file_index_lists_materialized_and_skipped_entries(tmp_path: Path) -> None:
    ...
    index = (workspace.input_dir / "documents" / ".skillmind" / "files.txt").read_text(
        encoding="utf-8"
    )
    ...
    manifest = json.loads(
        (workspace.input_dir / "documents" / ".skillmind" / "manifest.json").read_text(
            encoding="utf-8"
        )
    )
    assert [item["path"] for item in manifest["generated"]] == ["documents/.skillmind/files.txt"]
...
async def test_repository_history_is_materialized_with_its_limit(tmp_path: Path) -> None:
    ...
    history = (
        workspace.input_dir / "source_repository" / ".skillmind" / "history.txt"
    ...
    manifest = json.loads(
        (workspace.input_dir / "source_repository" / ".skillmind" / "manifest.json").read_text(
            encoding="utf-8"
        )
    )
    assert {item["path"] for item in manifest["generated"]} == {
        "source_repository/.skillmind/files.txt",
        "source_repository/.skillmind/history.txt",
    }
...
async def test_generated_artifacts_are_verified_on_reuse(tmp_path: Path) -> None:
    ...
    index = workspace.input_dir / "documents" / ".skillmind" / "files.txt"
    index.chmod(0o600)
...
async def test_materialize_returns_descriptors_for_brief_and_prompt(tmp_path: Path) -> None:
    ...
    assert documents.index_path == "input/documents/.skillmind/files.txt"
    assert documents.manifest_path == "input/documents/.skillmind/manifest.json"
    ...
    assert repository.history_path == "input/source_repository/.skillmind/history.txt"
