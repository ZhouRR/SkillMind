...
from skillmind.auth.login_protection import LoginRateLimitedError
from skillmind.auth.service import AuthService, CsrfRejectedError, UnauthorizedSessionError
...
def database(monkeypatch: pytest.MonkeyPatch) -> SessionDatabase:
    ...
    monkeypatch.setattr("skillmind.auth.service.datetime", SimpleNamespace(now=lambda zone: NOW))
...
async def test_expiry_crossed_during_either_lock_is_not_extended(
    ...
    monkeypatch.setattr(
        "skillmind.auth.service.datetime", SimpleNamespace(now=lambda zone: clock.value)
    )
...
async def test_login_freezes_protocol_role_and_caps_initial_idle(database: SessionDatabase) -> None:
    ...
    assert result.session_token.startswith("sm2.") and result.csrf_token.startswith("csrf2.")
...
async def test_successful_logins_do_not_reset_real_redis_quota(
    ...
    assert len(await isolated_redis.keys("skillmind:auth:limit:v2:*")) == 3
