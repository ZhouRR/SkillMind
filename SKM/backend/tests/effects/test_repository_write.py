...
from skillmind.effects.catalog import EFFECT_CAPABILITIES, resolve_effect_capability
...
def _draft(**overrides: Any) -> ChangeProposalDraft:
    ...
    fields: dict[str, Any] = {
        ...
        "operation": "commit",
        "target": {
            "locator": "skillmind/run-7a1c/fix-null-guard",
            "display": "skillmind/run-7a1c/fix-null-guard",
        },
        "changes": (
            {
...
def test_valid_proposal_normalizes_into_a_provider_payload() -> None:
    ...
    assert payload["target_branch"] == "skillmind/run-7a1c/fix-null-guard"
...
def test_branch_mode_still_requires_the_reserved_namespace() -> None:
    ...
    with pytest.raises(ChangeProposalValidationError, match="skillmind/ namespace"):
...
def test_write_prefix_narrows_within_the_reserved_namespace() -> None:
    ...
    config = {**_BRANCH_CONFIG, "write_branch_prefix": "skillmind/review/"}
    ...
    payload = _validate(
        _draft(target={"locator": "skillmind/review/fix", "display": "skillmind/review/fix"}),
        integration_config=config,
    )
