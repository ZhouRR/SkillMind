...
def test_percent_encoded_password_survives_alembic_config() -> None:
    ...
    database_url = "postgresql+asyncpg://skillmind:P%40ssword@postgres:5432/skillmind"
    config = Config()
...
def test_run_skill_snapshot_migration_only_creates_the_binding_schema(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """公開 Skill や仮の Run snapshot を挿入せず、必要な表と索引だけを作成する。"""
    ...
    migration.upgrade()
    assert operations.create_table.call_args.args[0] == "run_skill_snapshots"
    ...
    migration.downgrade()
    operations.drop_table.assert_called_once_with("run_skill_snapshots")
    operations.execute.assert_not_called()
    operations.get_bind.assert_not_called()


def test_new_installation_offline_sql_creates_schema_without_business_seeds(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """全 revision を DB 接続なしで適用し、組織以外の固定業務 data を投入しない。"""

    buffer = io.StringIO()
    backend_dir = Path(__file__).resolve().parents[2]
    config = Config(str(backend_dir / "alembic.ini"), output_buffer=buffer)
    ...
    assert set(re.findall(r"CREATE TABLE (\w+)\s*\(", sql)) == {
        *Base.metadata.tables,
        "alembic_version",
    }
    assert set(re.findall(r"INSERT INTO (\w+)\b", sql)) == {
        "alembic_version",
        "organizations",
        "project_skill_versions",
    }
    assert sql.count("INSERT INTO organizations") == 1
    assert f"'{SYSTEM_ORGANIZATION_ID}', 'Skillmind', 'ACTIVE'" in sql
...
def test_migration_chain_has_a_single_expected_head() -> None:
    ...
    assert scripts.get_revision("0025_task_schedules").down_revision == "0023_managed_secrets"
