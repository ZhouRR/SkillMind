...
from skillmind.schedules.repository import _to_record
...
async def test_expiry_after_flush_rolls_back_settlement_and_count() -> None:
    ...
    with (
        patch(
            "skillmind.schedules.repository_occurrences._now",
...
async def test_overlap_uses_all_associations_not_last_run_pointer(status: str) -> None:
    ...
    with patch("skillmind.schedules.repository_occurrences._now", return_value=NOW):
...
async def test_claim_lease_begins_after_waiting_for_schedule_lock() -> None:
    ...
    with patch("skillmind.schedules.repository_occurrences._now", return_value=after_wait):
...
async def test_status_transition_is_revalidated_after_lock() -> None:
    ...
    from skillmind.schedules.domain import InvalidScheduleTransitionError
