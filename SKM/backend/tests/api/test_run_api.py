...
from tests.api.fakes import FakeAuthService, FakeRunService, FakeSkillService
...
def test_create_task_run_returns_server_snapshot_identity(client: TestClient) -> None:
    ...
    assert response.headers["Location"] == (f"/skillmind/api/v1/runs/{response.json()['run_id']}")
