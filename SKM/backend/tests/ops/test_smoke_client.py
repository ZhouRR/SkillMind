...
from skillmind.ops import smoke
from skillmind.ops.smoke import SmokeApiClient, SmokeTask
from tests.ops import acceptance_fakes as acceptance
...
def test_smoke_login_replays_secure_cookie_and_csrf_on_internal_http(
    ...
    responses = iter((
        FakeHttpResponse(
            ...
            ("skillmind_login_csrf=login-token; Secure; HttpOnly; Path=/",),
        ),
        FakeHttpResponse(
            '{"csrf_token":"session-csrf","user":{}}',
            (
                "__Host-skillmind_session=session-token; Secure; HttpOnly; Path=/",
                "skillmind_login_csrf=; Max-Age=0; Path=/",
            ),
        ),
    ...
    assert unsafe_request.get_header("Cookie") == "__Host-skillmind_session=session-token"
...
def test_smoke_password_file_requires_owner_only_permissions(
    ...
    monkeypatch.setenv("SKILLMIND_SMOKE_PASSWORD_FILE", str(password_file))
...
def test_smoke_password_never_uses_environment_value(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("SKILLMIND_SMOKE_PASSWORD_FILE", raising=False)
    monkeypatch.setattr(smoke.sys.stdin, "isatty", Mock(return_value=False))
    monkeypatch.setenv("SKILLMIND_SMOKE_PASSWORD", "must-not-be-read")

    with pytest.raises(RuntimeError, match="TTY or SKILLMIND_SMOKE_PASSWORD_FILE"):
        smoke._read_smoke_password()
