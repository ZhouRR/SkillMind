...
from skillmind.api.problems import ProblemException, problem_exception_handler
from skillmind.api.routes.skills import PublishSkillVersionRequest, publish_skill_version
...
async def test_saved_association_and_original_bytes_are_rechecked(case: str) -> None:
    ...
    if case == "manifest-hash":
        ...
        session.manifest.manifest_version = "skillmind/unknown"
    before = session.frozen_values()
    with pytest.raises(SkillPublishGateError):
        await session.publish()
    assert session.frozen_values() == before
