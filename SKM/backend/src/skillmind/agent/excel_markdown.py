...
_THEME_KEYS = ("lt1", "dk1", "lt2", "dk2", "accent1", "accent2", "accent3", "accent4",
...
class _CellTokens:
    ...
    def segment(self, text: str, strike: bool) -> str:
        ...
        return ((self.add(text[:left]) if left else "")
                + "<del>" + self.add(text[left:right]) + "</del>"
...
class _Colors:
                ...
                if scheme is not None:
                    for index, key in enumerate(_THEME_KEYS):
                        ...
                        if node is not None and len(node):
                            ...
                            if not len(color):
                                value = color.get("val") if color.tag.endswith("}srgbClr") else (
                                    color.get("lastClr") if color.tag.endswith("}sysClr") else None
                                )
    ...
    def describe(self, color: Color) -> dict[str, Any]:
        ...
        if color.tint:
            ...
            luminance = (luminance * (1 + color.tint) if color.tint < 0
    ...
    def fill(self, fill: Fill) -> dict[str, Any]:
        ...
        if isinstance(fill, GradientFill):
            return {"type": "gradient", "gradient": fill.type, "degree": fill.degree,
                    "left": fill.left, "right": fill.right, "top": fill.top, "bottom": fill.bottom,
                    "stops": [{"position": stop.position, "color": self.describe(stop.color)}
                              for stop in fill.stop]}
        # openpyxl の StyleProxy も同じ公開 fill 属性を提供する。
        pattern = getattr(fill, "patternType", None)
        if pattern is None:
            return {"type": "none"}
        return {"type": "pattern", "pattern": pattern,
                "foreground": self.describe(getattr(fill, "fgColor")),
                **({"background": self.describe(getattr(fill, "bgColor"))}
                   if pattern != "solid" else {})}
...
def _style_facts(sheet: Worksheet, colors: _Colors) -> dict[str, Any]:
    ...
    facts: dict[str, Any] = {"profile": STYLE_PROFILE, "cell_styles": [
        {"ranges": _compact_ranges(coords), **json.loads(detail)}
        for detail, coords in sorted(groups.items())
    ]}
    if rich_strikes:
        facts["rich_text"] = rich_strikes
        facts["rich_text_offsets"] = "Zero-based Unicode code points, end exclusive; overrides base_font_strike."
    defaults = []
    for kind, dimensions in (("row", sheet.row_dimensions), ("column", sheet.column_dimensions)):
        for key, dimension in sorted(dimensions.items(), key=lambda entry: str(entry[0])):
            if dimension.has_style:
                defaults.append({"axis": kind, "key": str(key),
                                 **({"min": dimension.min, "max": dimension.max}
                                    if kind == "column" else {}),
    ...
    if merges:
        ...
        facts["merge_note"] = "Value and static style refer to the anchor; covered cells are not copied."
    conditional = []
    for region in sheet.conditional_formatting:
        rules = []
        for rule in sheet.conditional_formatting[region]:
            entry: dict[str, Any] = {"type": rule.type, "priority": rule.priority,
                                     "stop_if_true": rule.stopIfTrue, "operator": rule.operator,
            ...
            rules.append(entry)
        conditional.append({"range": str(region.sqref), "evaluation": "NOT_EVALUATED", "rules": rules})
    if conditional:
        facts["conditional_formats"] = conditional
    if sheet.tables:
        facts["table_styles"] = [{"range": table.ref, "evaluation": "NOT_EVALUATED"}
...
def _rich_segments(value: Any, strike: bool) -> list[tuple[str, bool]]:
    ...
    for part in value if isinstance(value, CellRichText) else (str(value),):
        ...
        flag = (part.font.strike if isinstance(part, TextBlock) and part.font.strike is not None
                else strike)
...
def _sheet_table(
    rows: Sequence[Sequence[str]], sheet: Worksheet | None,
    html_to_markdown: Callable[[str], str],
) -> str:
    """原行/列を表中に示し、先頭行を header 推論で改名/消失させない。"""
    tokens = _CellTokens()
    width = max((len(row) for row in rows), default=0)
    output = ["<table><thead><tr><th>Excel row</th>" + "".join(
        f"<th>{get_column_letter(column)}</th>" for column in range(1, width + 1)
    ) + "</tr></thead><tbody>"]
    for row_number, values in enumerate(rows, start=1):
        ...
        for column, value in enumerate(values, start=1):
            ...
            output.append("<td>" + "".join(tokens.segment(text, flag) for text, flag in segments) + "</td>")
        output.append("</tr>")
...
def render_styled_sheets(
    data: bytes, sheets: Mapping[str, Any], html_to_markdown: Callable[[str], str],
) -> str:
    ...
    limitations = ["Conditional formatting and table styles are not evaluated; static fill is not "
    ...
    try:
        ...
        if caught:
            limitations.append("Workbook reader reported unsupported features; visual fidelity is incomplete.")
    except (ValueError, TypeError, KeyError, IndexError, OSError, ElementTree.ParseError):
        limitations.append("STYLE_EXTRACTION_UNAVAILABLE: only cell values were read; do not infer "
    ...
    try:
        ...
        for name, frame in sheets.items():
            ...
            facts = _style_facts(sheet, colors) if sheet is not None and colors is not None else {
                "profile": STYLE_PROFILE, "style_status": "NOT_INSPECTED"
            }
            facts["limitations"] = limitations
            encoded = json.dumps(facts, ensure_ascii=False, indent=2)
            # 原式に backtick が含まれても code fence の外へ出ない。
            fence = "`" * max(3, 1 + max((len(run) for run in re.findall(r"`+", encoded)), default=0))
            sections.append(f"## {markdown_literal(name)}\n{body}\n\n"
                            f"### Excel style facts\n\n{fence}json\n{encoded}\n{fence}")
        return "\n\n".join(sections).strip()
    finally:
        if book is not None:
