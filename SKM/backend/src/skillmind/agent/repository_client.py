...
from skillmind.integrations.domain import REPOSITORY_URI_SCHEMES
...
_COMMIT_AUTHOR_NAME = "Skillmind"
_COMMIT_AUTHOR_EMAIL = "skillmind@skillmind.local"
...
SVN_BRANCH_ROOT = "branches/skillmind"
...
class GitCommandRepositoryClient:
    ...
    async def open(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="skillmind-git-") as workspace:
            root = Path(workspace)
            home = root / "home"
    ...
    async def open_writable(
        ...
        *,
        ...
        _validate_uri("git", uri)
        _validate_revision(revision)
        with tempfile.TemporaryDirectory(prefix="skillmind-git-write-") as workspace:
            root = Path(workspace)
            home = root / "home"
            home.mkdir(mode=0o700)
            checkout = root / "repository"
...
class SvnWriteSession:
    ...
    svn の「branch」は server 側 copy であり、その置き場は仓库布局に依存する。Skillmind は
    **平台固定の約定**として `<repository root>/branches/skillmind/<名前>` を使う (root は
    ...
    async def commit_files(
        ...
        with tempfile.TemporaryDirectory(prefix="skillmind-svn-write-") as workspace:
...
class SvnCommandRepositoryClient:
    ...
    async def open(
        ...
        *,
        ...
        _validate_revision(revision)
        with tempfile.TemporaryDirectory(prefix="skillmind-svn-") as workspace:
            root = Path(workspace)
