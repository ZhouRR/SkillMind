...
class SmokeApiClient:
    ...
    def __init__(self, base_url: str) -> None:
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise ValueError("SKILLMIND_SMOKE_BASE_URL must be an absolute HTTP(S) URL")
...
def _resolve_smoke_task(client: SmokeApiClient, project_id: UUID) -> SmokeTask:
    ...
    requested_version = os.getenv("SKILLMIND_SMOKE_SKILL_VERSION_ID", "").strip()
    requested_task_key = os.getenv("SKILLMIND_SMOKE_TASK_KEY", "").strip()
    ...
    raw_sources = _read_json_object("SKILLMIND_SMOKE_TASK_SOURCES_JSON", "{}")
    if not all(isinstance(value, str) for value in raw_sources.values()):
        raise RuntimeError("SKILLMIND_SMOKE_TASK_SOURCES_JSON values must be strings")
    return SmokeTask(
        skill_version_id=skill_version_id,
        task_key=task_key,
        input_json=_read_json_object("SKILLMIND_SMOKE_TASK_INPUT_JSON", "{}"),
...
def resolve_smoke_project_id() -> UUID:
    ...
    configured = os.getenv("SKILLMIND_SMOKE_PROJECT_ID")
    if not configured:
        raise SystemExit(
            "SKILLMIND_SMOKE_PROJECT_ID is required: set it to the project that owns "
...
def _read_smoke_email() -> str:
    ...
    email = os.getenv("SKILLMIND_SMOKE_EMAIL", "").strip()
    if not email:
        if not sys.stdin.isatty():
            raise RuntimeError("SKILLMIND_SMOKE_EMAIL is required without an interactive TTY")
...
def _read_smoke_password() -> str:
    ...
    password_file = os.getenv("SKILLMIND_SMOKE_PASSWORD_FILE")
    if password_file:
        ...
        if not sys.stdin.isatty():
            raise RuntimeError(
                "Interactive TTY or SKILLMIND_SMOKE_PASSWORD_FILE is required for smoke login"
            )
...
def main() -> None:
    ...
    base_url = os.getenv("SKILLMIND_SMOKE_BASE_URL", "http://127.0.0.1:8000")
    timeout_seconds = int(os.getenv("SKILLMIND_SMOKE_TIMEOUT_SECONDS", "1200"))
    ...
    print("Skillmind generic task smoke passed")
