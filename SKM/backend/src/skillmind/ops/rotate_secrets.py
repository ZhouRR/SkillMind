...
`SKILLMIND_MANAGED_SECRET_KEK` を「新 active 鍵 + 旧鍵」の keyring にした状態で実行する。
...
from skillmind.core.secret_crypto import SecretCryptoError, load_secret_cipher
...
async def rotate_managed_secrets() -> tuple[int, int]:
    ...
    if cipher is None:
        raise SecretCryptoError("SKILLMIND_MANAGED_SECRET_KEK is not configured")
