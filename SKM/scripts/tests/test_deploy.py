...
PROBE = ("python", "-m", "skillmind.ops.preflight")
START = (
    "up",
    "-d",
    "--no-build",
...
class DeploymentTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        directory = tempfile.TemporaryDirectory(prefix="skillmind-deploy-test-")
        ...
        for phase in ("migrate", "api", "worker"):
            with self.subTest(phase=phase):
                ...
                def start_writer(fake: FakeContext, entry: Trace) -> None:
                    ...
                    if entry[0] == "compose" and "skillmind.ops.preflight" in entry[1]:
                        fake.rows.append(container("worker", 9))
