...
async def check_consolidated_entries(page: Page, book: Path) -> None:
    ...
    for width in (390, 1440):
        ...
        for module in MODULES:
            await page.goto(page_url(book, f"SKM/{module}/README.md", "旧章"))
            await expect(page).to_have_url(page_url(book, "SKM/README.md", module))
            await check_heading(page)
            await page.reload()
            await check_heading(page)
...
async def check_navigation(page: Page, book: Path, pages: list[dict]) -> None:
    ...
    await expect(page.locator("#main h1")).to_have_text("Skillmind 文書ガイド")
    ...
    await page.goto(page_url(book, "SKM/README.md", "backend"))
    await page.locator('#toc-links a[href="#SKM/README.md::web"]').click()
    await check_heading(page)
    await page.go_back()
    await expect(page).to_have_url(page_url(book, "SKM/README.md", "backend"))
    await check_heading(page)
    await page.go_forward()
    await check_heading(page)
    await expect(page).to_have_url(page_url(book, "SKM/README.md", "web"))
