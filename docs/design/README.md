...
| 時刻を指定して起動し、保存後の変更を扱う | [TaskSchedule](task-scheduling.md) → [共有入力の UI](workspace.md#82-操作) → [Run 作成](run-creation.md)。編集 API と画面、規則時区と端末時区を区別する |
...
| [受控書き込み](repository-effects.md) | Proposal、精確批准、CAS、apply、read-back、部分失敗 | ユーザーへの表示は Workspace、配備復旧は Runbook |
| [TaskSchedule](task-scheduling.md) | 発火時刻、保存した配置、occurrence、遅延・計数・復旧 | Run は普通の作成サービスを使い、独自 engine を持たない |
