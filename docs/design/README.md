...
設計は規則と未実装の目標、[contracts](../../SKM/README.md#contracts) は正確な field/type、計画は現在の範囲と不足を記す。設計があるだけで実装済み・今回必須とは扱わない。

既存機能の修正は該当する規則と回帰だけを適用する。「目標」「接続待ち」「验收」は、その能力を実装・公開する際の条件であり、日常修正で全項目を完成させる指示ではない。ただし既に実行可能な入口の権限・停止・未知結果の保護は、後置扱いだけでは解除できない。
...
| [解釈の安定性](skill-interpretation.md) | 入力 root・調整 scope・実効設定の凍結 |
