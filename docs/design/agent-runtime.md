...
新 Run 固定 `runtime_policy=skillmind.runtime/v4`，该字段进入 AgentTaskBrief、checksum 和原生会话指令比较。OutcomeEnvelope 继续返回完整结论、依据、范围、限制、交付物与引用，由页面统一排版，默认不再要求模型额外生成整页 HTML/CSS 或重复执行履历。v3 进一步要求在现有字段中使用简明 Markdown，正文优先结论、依据和范围，不重复 UUID、hash、存储坐标或回执；必需业务标识和精确引用仍保留在原结构化字段/成果中。v4 另外将实际配置的模型 ID 作为 `runtime_metadata.model` 提供给 Agent 并纳入 Brief checksum；它不代表不可变的供应商内部构建号。`wall_timeout_scope=AGENT_ATTEMPT` 明确 Agent 局部期限不是整场桌面测试期限。v2/v3 与未记录 policy 的历史 Run 保持原报告指令，不在续行时升级。Skill 明确要求的业务 HTML/Markdown 等成果仍按原规则生成、校验和保存。未记录 runtime policy 的旧 Run 保留原 HTML 提示；新版展示端同时读取旧 HTML 与无 HTML 的结构化结果，不改历史 checksum。平台页面区分执行状态、结果完成程度与 Skill 的业务判定；不会将 SUCCEEDED 推导为 PASS，也不会登记新的业务成果。附加排版失败时退回原结果展示，不重跑模型、不改已保存的业务状态。
...
当前 `change.propose` 仍以原事务释放 Agent Attempt、进入 Effect 等待并在结果确认后新建 Segment。不能仅保持 SDK callback 打开而复用已释放的 lease。后续连续返回回执需要确保：提交原 Proposal/Approval/Effect 后才交付；同一结果只选择 inline 或持久续行一个去向；当前授权、取消、限额和 UNKNOWN 照合不变；等待 Effect 不依赖被 Agent 占满的同一执行槽。

短序列只考虑参数已确定的有限严格顺序调用，保留每步批准和只读回执。回执确认与允许后续操作分开：工具已返回终态 ERROR/TIMEOUT 时，Effect 可已确认，但继续条件失败必须停止。未知结果仅查询原操作，不换 ID 重发。游标、结果和下一调度需原子持久化；不从对话重建状态，不添加表达式、自动分支、业务工具名称判断或补偿。

本次只在 test-only harness 中验证上述局部性质，不新增生产入口、配置开关或状态。SDK 原调用关联、多个子 Proposal、原 PG 事务锁序、Outbox 双重交付、跨实例竞争和预算仍须完成后，才能启用连续执行；暂不将隔离试验接入正常任务。
