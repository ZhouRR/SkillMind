...
任务中心选择“执行什么”，Workspace 集中启动任务、处理待办和阅读报告；执行历史负责记录列表及单次 Run 的完整详情。本页负责布局与交互；提交协议见[创建](run-creation.md)、[答复](user-interactions.md)、[批准](repository-effects.md)、[评价](results-evaluation.md)。当前缺口统一见[计划 R10](../planning/roadmap.md#开发任务)。
...
- **信息密度**：执行列表以原 Run 绑定的任务名称为标题，时间与状态单列，结果摘要作为正文；缺少历史名称时回退摘要或通用任务标题，不以 UUID 充当名称。概览突出待办与最近执行，技能默认列表、导入另开页签，任务突出执行入口。ID/版本/校验值收进技术详情，空闲的上传原请求查询与停止待发布操作收进帮助/高级入口；进行中的审批、失败和结果未知仍直接可见。
...
```text
Workspace：新建执行、待处理、执行中、已完成与报告
├── 当前任务：状态、取消、答复与批准
└── 结果报告：任务选择、最新执行的总结、执行详情入口
执行历史：分页列表 → 单次执行详情
└── 状态、AI 对话、完整结果与附件、证据、人工评价、工具记录、审计事件
```

待处理/执行中按服务端状态筛选并分页，不能过滤首页冒充全量。已完成与报告改为当前模块的任务下拉选择，按精确 Task ID 在服务端筛选最新终态 Run（创建时间、ID 倒序），直接展示报告；没有结束执行时显示空态，最新失败或取消且无结果时明确显示该次状态，不回退展示旧成功报告，不推测测试数量或业务通过率。切换任务或项目立即隔离旧请求和内容。概要最近执行/待办、Workspace 工作列表、报告详情链接、定期执行的上次执行及历史列表，统一在 `#/history?project=…&run=…` 打开单次详情，侧栏归属执行历史；旧 `#/workspace?…&run=…` 链接也按此归属解析。刷新保留原 Run，点击 Workspace 或其模块返回工作列表。任务启动链接仍进入 Workspace；各入口共用执行、答复、批准和评价的原身份/请求状态实现。

Workspace 的待处理、执行中和报告 Tab 保持同一宽度与启动入口布局；报告仅展示任务总结、执行状态、耗时、复核标识和执行详情入口，不挂载附件、评价或技术明细。完整结果、答复、批准、未知效果和限制在执行详情呈现。详情中的 Outcome 完整 HTML 报告直接静态渲染，保留内嵌 CSS 和 SVG；嵌入阅读时统一缩小页面及顶层内容区的留白，让正文利用可用宽度，原打印样式保留。Markdown/纯文本成果不因 `kind=report` 进入 iframe，而复用自适应高度、随主题的阅读组件；完整 HTML 保留文档 sanitizer 和无脚本 sandbox，禁止外部资源及导航。公开附件先校验原 Run 索引及原 hash/大小，再按相同格式分流预览；原 Result 不改写。JSON 预览仅格式化空白，保留数值精度、重复字段和原字符串；不合法或过深内容按原文显示，下载仍使用原字节。未公开或超出预览限额的附件不推测内容，保留下载/不可用状态。源码见 [WorkspacePage](../../SKM/web/src/pages/WorkspacePage.tsx)。
...
摘要、交付、Findings、Evidence、限制、Proposal/Effect、Evaluation 分区；confidence 不是正确率。通用任务总结由平台固定排版，摘要作为正文完整展示，Markdown 的标题、表格、列表与代码按统一阅读样式渲染，不新增模型调用或业务附件。证据抽屉展开摘录后默认预览，可切换完整原文；关闭的摘录不提前解析。证据以可确认的文件名/表名、类型、时间及原阶段区分；定位 JSON、ID/hash 收入技术明细，无摘录时直接展开位置。历史成果正文及长描述可展开完整查看，已完成效果和运行诊断默认折叠，未解决异常仍带明确提示；空问题区不显示，限制始终保留。附件友好名只能由精确原证据/哈希/转换源与冻结文档的关联，或准确 artifact_ref 对应的成果标题取得，否则回退原文件名；预览入口仅经公开索引准确解析 artifact_ref，不从路径或相似名称推测。内联 Markdown 的原始 HTML 按文本显示，链接和图片只保留文字，不执行脚本、样式或外部请求；旧完整 HTML 报告仍走隔离预览。附件按同 Project/Run 发布索引区分采用/未采用，无保存记录不可下载；保持保存时 v1/v2 原义，字节与请求校验见[可信附件](results-evaluation.md#可信附件的发布与读取)。
...
执行履历、概要的最近执行、工作区队列、完了・レポート及任务卡片共同显示耗时。已结束取 `finished_at - started_at`，执行中按当前时间更新；未开始显示未开始，缺失或无效历史时间不伪造 0 秒。该总耗时包含开始后的审批/回答等待，不含开始前排队，不称为模型推理耗时。
