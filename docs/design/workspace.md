...
├── 当前任务：状态、取消、答复与批准
└── 结果报告：摘要、发现事项、人工评价、Markdown/HTML 附件预览
执行历史：分页列表 → 单次执行详情
└── 状态、AI 对话、结果与证据、工具记录、审计事件
```
...
Workspace 默认不展示 Run ID、连接协议、工具和审计记录；答复、批准、未知效果和限制仍在报告附近直接呈现。完了报告采用全宽阅读区，启动入口收为上方按钮，队列页保持原布局；Outcome 的完整 HTML 报告直接静态渲染，保留内嵌 CSS 和 SVG；嵌入阅读时统一缩小页面及顶层内容区的留白，让正文利用可用宽度，原打印样式保留。Markdown/纯文本成果不因 `kind=report` 进入 iframe，而复用自适应高度、随主题的阅读组件；完整 HTML 保留文档 sanitizer 和无脚本 sandbox，禁止外部资源及导航。公开附件先校验原 Run 索引及原 hash/大小，再按相同格式分流预览；原 Result 不改写。JSON 预览仅格式化空白，保留数值精度、重复字段和原字符串；不合法或过深内容按原文显示，下载仍使用原字节。未公开或超出预览限额的附件不推测内容，保留下载/不可用状态。源码见 [WorkspacePage](../../SKM/web/src/pages/WorkspacePage.tsx)。
