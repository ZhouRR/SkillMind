...
目前须保留用户管理 OpenAPI 快照未接齐、账户组件未接 App、共享预算未接执行等缺口；文档构建不消除这些问题。

### 下一步与当前决策

1. 先收口身份/资源边界、Run 停止与事务、共享预算和调度可靠性，再扩大自主执行能力。
2. 接齐已有账户组件和契约，完善普通答复、评价、项目/文档管理的未知结果与上下文隔离。
...
- 状态：会话 v2/0031、登录防护、用户 API/Schema/client 与项目 CRUD 已有代码；账户 page/route 和 OpenAPI 未接齐，账户组件/hook/文案草稿需保留接续。
...
- 状态：原请求确认、文档选择/详情、调度输入及登录已有局部基础；账户/成员入口、失效 Project 深链接、App 会话与多类 mutation 隔离待收口。
