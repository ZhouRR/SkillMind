...
- 状态：选择/清单、输入回执、命名空间独占、多根与世代隔离已有实现；单文档删除已接原授权与 Run/Schedule/保留 occurrence 引用门禁，调度保存锁内复查原文档。普通下载与冻结读取已共用有界实际字节校验，缺失/损坏/不可用分开拒绝。真实事务、迁移恢复及完整外部执行链待验。
...
- 验收：不越权/漂移、重放兼容、可信 byte 与跨根限额；补跨存储提交、并发配额、引用门禁的真实竞争及大规模扫描、真实存储故障/完整性验收与持久清理，关联 R05 的撤权/归档竞争。
