...
- 状态：导入/解释/发布/启停已有实现；DRAFT、首次发布和只读预览共用原身份、全部 Task 与来源门禁。草稿/发布/废弃/删除/项目启停已接原会话及当前 ADMIN 的业务事务复核，原审计值不重写；删除保护纳入调度、occurrence 和生成模块引用，新 Run/调度保存共用当前可用性检查。候选持久绑定、导入/解释写入资格、组合保存的组织/当前可用性门禁、真实事务、同版重新启用审计及回滚仍待补齐；跨 Skill 规则组合尚未实现。
