...
- 状态：选择/清单、输入回执、命名空间独占、多根与世代隔离已有实现；单文档删除已接原授权与历史引用门禁，调度保存锁内复查原文档。下载与冻结读取共用有界实际字节校验。上传已接鉴权后有界 multipart、锁外 PUT 前后原资格复核与发布时元数据用量复查；这不是持久预留/清理。真实事务、迁移恢复及完整外部执行链待验。
...
- 验收：不越权/漂移、重放兼容、可信 byte 与跨根限额；接续持久原上传意图、并发配额预留/结算、跨存储提交未知及阻止迟到 PUT 的精确清理，单文档/整项目与恢复共同接线。补授权/引用/配额的真实竞争、大规模扫描和存储故障验收，关联 R05。
...
- 状态：原请求确认、文档选择/详情、调度输入及登录已有局部基础；账户、成员、项目 CRUD 与普通答复共用防重/期限/旧响应隔离，各自定义拒绝和未知核对。单文档删除已接精确 204、原 ID 只读核对与人工解除；预览已接实际 byte 上限、静态 HTML 隔离、资格/晚到响应门禁及三语。App 保留精确项目边界。module、上传、评价与批准页面的完整 mutation/未知结果隔离仍待收口。
