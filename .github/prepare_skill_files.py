...
edit(p, '        sealed = tuple(sorted(files, key=lambda item: item.path))', '''        # 同じ世代・予算・完了回执へ加える。Schema も script も原 text byte のまま扱う。
        if skill_contents:
...
edit(p, 'workspace.write 只写 workspace/output；', '''冻结 Skill 文本不由模型复制：ContextBuilder 在原 Manifest 验证及工具授权后，将已有 `source_documents` 交给同一输入准备器。只有本 Run 已有 workspace read/search 或 JSON Schema 校验能力时才物化；不新增能力、用户参数、资源绑定或外部请求。所有冻结文本（包括 Schema、参考资料及脚本文本）保留原 UTF-8 字节、包内路径与 hash，写到保留的 `input/.skillmind/skill/<source_path>`。不重排 JSON、转换换行、去掉说明、执行脚本，未冻结的二进制文件也不冒充已提供。

文件与普通输入共享世代、现有单文件/单根/总量上限和 PREPARING → READY 事务；Brief 的可选 `skill_files` 只在完成后列出原路径、实际逻辑路径和 hash，并参与 Brief checksum。校验工具直接使用该路径作为 `schema_path`，无需模型重新输出 Schema；路径不是新权限，外部引用处理不变。重试/续行核对原回执、完整树和原文哈希后复用，不重复写文件。既有 READY 未含 Skill 文件时不改写、不新增启动阻断，继续原来的完整原文路径且不发布不存在的文件位置；缺失或被篡改的已封存文件仍按原完整性规则拒绝。无物化器或无相应能力时也不猜路径。原 Skill 的业务执行、审批和成果保存规则不变。
