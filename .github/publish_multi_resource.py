"""全回帰の内容一致後にだけ review branch を進め、main と実行環境を変更しない。"""
...
for path in ('.github/prepare_multi_resource.py', '.github/publish_multi_resource.py',
             '.github/fix_multi_resource.py', '.github/workflows/multi-resource-review.yml'):
    if Path(path).exists():
...
commit = api('git/commits', {'tree': tree['sha'], 'parents': [parent], 'message':
    'feat(resources): route same-capability tools by frozen resource key\n\n'
    'Expose one SDK tool with exact selectors; preserve singleton calls and each '
    'binding, evidence, replay identity and effect authorization. PostgreSQL, MCP '
    'and repository resources share the same routing without runtime network expansion.'})
assert api('git/ref/heads/' + branch)['object']['sha'] == parent
